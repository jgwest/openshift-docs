...
To manage progressive delivery of deployments by using Argo Rollouts in {gitops-title}, you must create and configure a `RolloutManager` custom resource (CR) in the namespace of your choice. By default, a new `argo-rollouts` instance has permission to manage resources across all namespaces on the cluster. 
